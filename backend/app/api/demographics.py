...
from formatters.age_formatter import format_demographic_results_into_age_results_for_county
from formatters.age_formatter import format_demographic_results_into_age_results_for_state
...
@bp.route('/get_age_trends_for_county/<string:fips>', methods=['GET'])
def get_age_trends_for_county(fips: str) -> Response:
    """
    Given a county fips, get the historical age data for that county
    """
    try:
        # input validation 
        if not isinstance(fips, str):
        ...
        # read demographic data from db. age data is within demographic data
        demographic_trends = get_demographic_trend_for_county(fips, county, state)

        # format into age trends
        data = format_demographic_results_into_age_results_for_county(demographic_trends)
        json_data = dfToJson(data)
...
@bp.route('/get_age_trends_for_state/<string:state>', methods=['GET'])
def get_age_trends_for_state(state: str) -> Response:
    """
    Given a state, get the historical demographic data for that state
    """
    try:
        ...
        # format into age trends
        state_demographic_trends = format_demographic_results_into_age_results_for_state(demographic_trends)
        json_data = dfToJson(state_demographic_trends)
